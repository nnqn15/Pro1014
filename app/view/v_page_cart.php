...
$cart=$data["cart"];
if(isset($data["has_cart"])){
    $has_cart=$data["has_cart"];
}
?>
<div class="container">
            ...
            <div class="cart-table-container">
                <table class="table table-cart">
                    ...
                    <tbody>
                        <?php if(!$cart):?>
                            <tr>
                                <th class="text-center" colspan="5"><h3>Giỏ hàng trống!</h3></th>
                            </tr>
                        <?php else:?>
                            ...
                            <tr class="product-row total_product_parent">
                                <td>
                                    <figure class="product-image-container">
                                        <a href="<?= BASE_URL ?>product/detail/<?= $TenSP ?>" class="product-image">
                                            <img src="<?= BASE_URL ?>public/upload/products/<?= $AnhSP ?>" alt="<?= $TenSP ?>">
                                        </a>

                                        <a href="<?= BASE_URL ?>delete_cart&MaSP=<?= $MaSP ?>" class="btn-remove"
                                            title="Xóa sản phẩm"><i class="fa-solid fa-xmark"></i></a>
                                    </figure>
                                </td>
                                <td class="product-col text-left">
                                    <a href="<?= BASE_URL ?>product/detail/<?= $TenSP ?>" class="name_cart">
                                        <?= $TenSP ?>
                                    </a>
                                </td>
            ...
            <div class="cart-summary">
                ...
                <div class="checkout-methods">
                    <form action="<?= BASE_URL ?>checkout" method="post">
                        <input type="hidden" class="total_checkout" name="total_cart">
...
<script>
    $(document).ready(function () {
        function updateTotal() {
            ...
            document.querySelector('.total_checkout').value = total_cart;
        }
        updateTotal()
        ...
        $('input[name="quantity"]').on('change', function update_quantity() {
            ...
            $.ajax({
                type: "POST",
                url: '<?= BASE_URL ?>ajax_cart_quantity',
                data: {
